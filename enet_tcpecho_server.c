...
// Array to hold the commands passed from the client
char cmd[4096]= {0};

// Array to hold the result of temperature or pressure transformed into string
...
//                    cmd[cmd_index] = pcpayLoad[ui32index-ui32statuslen];
//
//                    UARTprintf("\n %c", cmd[cmd_index]);
//                    cmd_index++;
...
static err_t
echo_recv2( void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
    uint32_t ui32index;
    uint32_t ui32len;
    char *pcpayLoad;
    ...
    if (err == ERR_OK && p != NULL)
    {
       ...
       // usprintf(pcheaderMsg, "Server received %d bytes.\n", ui32len);
        ...
        for(cmd_index = 0; cmd_index < ui32len; cmd_index++){
            cmd[cmd_index] = 0;
        }

        for(ui32index = ui32statuslen; ui32index < 16+ui32statuslen;ui32index++){
                            g_pctcpBuffer[ui32index] = 0;
                        }
      ...
      //  char temp[4] = "temp";
       ...
       // tempCMD[4] = "TEMP";
        ...
        for(ui32index = ui32statuslen; ui32index < (ui32len + ui32statuslen); ui32index++){
            ...
            if(strncmp(cmd, tempCMD, 4 ) == 0 ){
                tempRead = TSYS01ReadTemperature();
                sprintf(tempResult, "%f", tempRead);
                match++;
                UARTprintf("\n INTO TEMP CMD");
                break;
            }
            else if(strncmp(cmd, pressureCMD, 4 ) == 0 ){
//                pressResult = MS5837readPressure();
//                sprintf(tempResult, "%f", pressResult);
//                match++;
//                UARTprintf("\n INTO PRESS CMD");
//                break;
               MS5837readPressure(pressSensorResult);
               for(count = 0; count < 4; count++){
                   UARTprintf("\n INTO PRESSURE CMD");
                   sprintf(tempBuffer, "%f", pressSensorResult[count]);

                //   UARTprintf("\n pressure buffer: %s", tempBuffer);
                   strcat(tempResult, tempBuffer);
                   strncat(tempResult, &comma, 1);
               }
               match++;
               break;
            }
            else if(strncmp(cmd, phCMD, 2 ) == 0 ){
               PHEZOReadPH(phBuffer);
               for(count = 1; count < 10; count++){
                   sprintf(tempBuffer, "%c", phBuffer[count]);
                   strcat(tempResult, tempBuffer);
               }
               match++;
               break;
        ...
        if(match > 0){
            count = 0;
            ui32len = strlen(tempResult);
            UARTprintf("\n INTO MATCH");

            for(ui32index = ui32statuslen; ui32index < (ui32len + ui32statuslen); ui32index++){
               g_pctcpBuffer[ui32index] = tempResult[count];
...
main(void)
{
    ...
    //
    // Initialize I2C protocol and sensors
    //
    InitalizeSensorsAndSystem(g_ui32SysClock);

    //
    // Initialize the UART and write initial status.
    //
    UARTStdioConfig(0, 115200, g_ui32SysClock);
    UARTprintf("\nEthernet lwIP TCP echo example.\n\n");
   ...
   // UARTprintf("Waiting for IP, %s", pui8MACArray);
    //
    // Initialize the lwIP library, using AUTO IP.
    //
    //lwIPInit(g_ui32SysClock, pui8MACArray, 0, 0, 0, IPADDR_USE_AUTOIP);

    lwIPInit(g_ui32SysClock, pui8MACArray, 0xA9FE99AD, 0xFFFF0000,0, IPADDR_USE_STATIC);
